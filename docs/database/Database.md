...
#### `official_baseline_snapshots` — Versiones importadas de baseline oficial

| Columna          | Tipo         | Restricciones         | Descripción                                      |
|------------------|--------------|-----------------------|--------------------------------------------------|
| `id`             | bigserial    | PK                    | Identificador de snapshot                        |
| `source_name`    | varchar(255) | NOT NULL              | Nombre del archivo importado                     |
| `source_path`    | varchar(255) | NOT NULL              | Ruta usada para importación                      |
| `source_type`    | varchar(20)  | NOT NULL              | Tipo de fuente (`csv`, `md`, etc.)              |
| `source_hash`    | varchar(64)  | NOT NULL              | Hash SHA-256 de la fuente                        |
| `imported_by`    | bigint       | FK → users.id         | Usuario que importó (nullable para CLI/sistema) |
| `is_current`     | boolean      | DEFAULT true          | Marca del snapshot vigente                       |
| `total_rows`     | integer      | DEFAULT 0             | Filas importadas                                 |
| `unique_domains` | integer      | DEFAULT 0             | Dominios normalizados únicos                     |
| `notes`          | text         | NULLABLE              | Observaciones operativas                         |
| `imported_at`    | timestamptz  | NULLABLE              | Fecha/hora de importación                        |
| `created_at`     | timestamptz  | NOT NULL              |                                                  |
| `updated_at`     | timestamptz  | NOT NULL              |                                                  |

#### `official_baseline_sites` — Filas baseline por snapshot

| Columna             | Tipo          | Restricciones                              | Descripción                                 |
|---------------------|---------------|--------------------------------------------|---------------------------------------------|
| `id`                | bigserial     | PK                                         |                                             |
| `snapshot_id`       | bigint        | FK → official_baseline_snapshots.id        | Snapshot al que pertenece                   |
| `row_number`        | integer       | NOT NULL                                   | Número de fila en fuente                    |
| `normalized_domain` | varchar(255)  | NULLABLE                                   | Dominio normalizado para reconciliación     |
| `classification`    | varchar(50)   | NULLABLE                                   | Clasificación                               |
| `entity`            | varchar(255)  | NULLABLE                                   | Entidad                                     |
| `site_name`         | varchar(255)  | NULLABLE                                   | Nombre del sitio                            |
| `domain`            | varchar(500)  | NULLABLE                                   | Dominio/URL original de fuente              |
| `is_active`         | boolean       | DEFAULT false                              | Estado activo proveniente de baseline       |
| `cms_label`         | varchar(120)  | NULLABLE                                   | Valor CMS textual de baseline               |
| `server_ip`         | varchar(80)   | NULLABLE                                   | IP/etiqueta de servidor                     |
| `certificate_label` | varchar(120)  | NULLABLE                                   | Texto histórico de certificado              |
| `project_status`    | varchar(120)  | NULLABLE                                   | Estatus de proyecto                         |
| `comments`          | text          | NULLABLE                                   | Comentarios                                 |
| `ticket_number`     | varchar(80)   | NULLABLE                                   | Ticket detectado en estatus/comentarios     |
| `raw_payload`       | jsonb/json    | NULLABLE                                   | Fila fuente completa para trazabilidad      |
| `created_at`        | timestamptz   | NOT NULL                                   |                                             |
| `updated_at`        | timestamptz   | NOT NULL                                   |                                             |

Regla de arquitectura:
- Estas tablas representan la baseline histórica importada.
- `sites` mantiene exclusivamente el estado operativo en tiempo real.
