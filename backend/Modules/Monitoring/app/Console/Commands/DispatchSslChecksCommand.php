...
final class DispatchSslChecksCommand extends Command
{
    protected $signature = 'monitoring:dispatch-ssl-checks
        {--limit=100 : Maximo de sitios a despachar por ciclo}
        {--chunk=40 : Tamano de lote para despacho gradual}
        {--stagger=1 : Segundos de desfase por lote para evitar rafagas}';

    protected $description = 'Despacha jobs de escaneo SSL para sitios monitoreados pendientes';
    ...
    public function handle(SiteRepositoryInterface $siteRepository): int
    {
        ...
        $sites = $siteRepository->dueForSslScan($limit);
        $dispatched = 0;
        ...
        $sites->chunk($chunkSize)->values()->each(function ($chunk, int $chunkIndex) use ($staggerSeconds, &$dispatched): void {
            ...
            foreach ($chunk as $site) {
                if ($delay !== null && ! $this->shouldDispatchMonitoringSynchronously()) {
                    RunSslCheckJob::dispatch((int) $site->id)->delay($delay);
                } else {
                    $this->dispatchMonitoringJob(RunSslCheckJob::class, (int) $site->id);
                }
